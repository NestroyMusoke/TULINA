...
- A Tulina Note is signed with ECDSA P-256/SHA-256, binds issuer key, recipient facility and device, transfer, batch, quantity, nonce, and expiry.
- Offline devices cache public keys only. Production private keys stay in Cloud KMS; local development uses an ephemeral signer.
