...
These JSON Schemas are the language-neutral boundary for facility, inventory, recommendation, audit, agent-run, and stock-card intake records. Strict Python and TypeScript models enforce the same invariants. Model-produced records are validated before a tool or action, and agent-run records intentionally exclude prompts and hidden model reasoning.
